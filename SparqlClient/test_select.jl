# Include the logger and SPARQL client modules
...
# Enable logging for monitoring the process
enable_logging()
log_info("Starting SELECT query test")
...
# Execute the query
log_info("Sending SELECT query...")
println("SELECT query result:")
result = query_and_convert(session)

# Log and print the JSON result
log_info("Received SELECT response")
println(result)
